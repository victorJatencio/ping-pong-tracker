    ...
    const fetchOpponents = async () => {
      ...
      try {
        ...
        if (fetchedOpponents.length === 0) {
          setErrorMessage(
            "No other users found. Make sure other users are registered in the system."
          );
        }
      } catch (error) {
    ...
    if (validateForm()) {
      try {
        ...
        const invitationData = {
          ...
          recipientId: formData.opponent, // Note: using recipientId to match your database structure
          status: "pending",
          message: formData.notes || "Let's play a ping-pong match!",
          scheduledDate: serverTimestamp(),
          location: formData.location,
          date: formData.date,
          time: formData.time,
          matchId: docRef.id, // Link to the created match
          createdAt: serverTimestamp(),
          updatedAt: serverTimestamp(),
        };

        console.log("Creating invitation data:", invitationData);
        ...
        setSuccessMessage(
          "Match successfully scheduled and invitation sent! You can update the score after playing."
        );
    ...
    <Modal show={show} onHide={handleClose} centered size="lg">
      ...
      <Modal.Body>
        ...
        <Alert variant="info" className="mb-3">
          <strong>📅 Scheduling a Match</strong>
          <br />
          You're creating a scheduled match with initial scores of 0-0. After
          playing, you can update the final scores from your dashboard.
        </Alert>
